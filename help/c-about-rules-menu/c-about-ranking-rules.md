...
1. 從以下選項中選擇：
   ...
   <table> 
    ...
    <tbody> 
      ...
      <li id="li_8DF5D8F924B24ECBBD2D93C76C69D00C"> 請確定您已在網站搜尋／銷售中設定Adobe Analytics驗證。 <p>請參 <a href="../c-about-settings-menu/c-about-adobe-analytics-menu.md#task_8AA93F6273B747F9B4DE9E8DFBCBDC42" type="task" format="dita" scope="local"> 閱設定Adobe Analytics量度驗證 </a>。 </p> </li> 
      <li id="li_CF7DD073FC5A432DADBD282AA8BB9920"> 選取並新增可用的報表套裝。 <p>請參 <a href="../c-about-settings-menu/c-about-adobe-analytics-menu.md#task_6DE17305EA7146DA8C30FF8FDF68A3C0" type="task" format="dita" scope="local"> 閱新增Adobe Analytics報表套裝 </a>。 </p> </li> 
      <li id="li_9A63448577D04E028DF211D8715F943A"> 設定您要用於建立新排名規則的Adobe Analytics量度清單。 <p>請參 <a href="../c-about-settings-menu/c-about-adobe-analytics-menu.md#task_360904CCBBB140238ADA036C3CC07664" type="task" format="dita" scope="local"> 閱編輯報表套裝的Adobe Analytics量度 </a>。 </p> </li> 
      <li id="li_1ACA3611D9B44AC394604CD89209C966"> 載入您網站頁面的初始Adobe Analytics量度。 <p>請參 <a href="../c-about-settings-menu/c-about-adobe-analytics-menu.md#task_2F3C55189B0A4049AB2113F2291CC181" type="task" format="dita" scope="local"> 閱載入Adobe Analytics資料 </a>。 </p> </li> 
      </ol> </p> </td> 
      </tr> 
    </tbody> 
    </table>
...
1. 新增一或多個排名規則。
   ...
   請參 [閱新增排名規則](../c-about-rules-menu/c-about-ranking-rules.md#task_A132789FD4E5423DAD090DCDA7311E8A)。

1. 按一 **[!UICONTROL regenerate your staged site index]** 下以對您的網站執行完整的重新索引(從 **[!UICONTROL Index]** > **[!UICONTROL Full Index]**)。
...
1. 檢查「 > >」中「排名」欄 **[!UICONTROL Settings]** 中的值 **[!UICONTROL Metadata]** , **[!UICONTROL Definitions]** 以確認您的排名規則已正確套用。

## 關於依年齡對檔案進行排名 {#topic_770815CF1B2A491F83FF765871B6F1B8}
...
變數 `H` 和 `T` 是此函式的輸入： `H` 是所需的半衰期， `T` 是檔案的年齡，以秒為單位。 `K` 是計算的半衰期， `RANK` 是指定年齡值的指數衰減。 結果值為0到1。 較新的檔案與較舊的檔案相比，其排名值更接近1。 理論上，檔案不應達到0，但捨入錯誤可能導致結果為0。

## 使用年齡排名的要求 {#section_D716507D589442C6B7848892BD28F966}

* 您的帳戶應已正確設定以進行排名。 如果未設定，請參閱設 [定排名](../c-about-rules-menu/c-about-ranking-rules.md#task_4CEBC13925B047FC95BDC217B48089C5)。
* HTML檔案必須有HTML meta標籤欄位，以表示其出生日期、開始日期（時間戳記）或某些其他有意義的日期值。
* 如「新增或編輯排名 `search_get_age_rank()`規則」頁面中所指定，特殊的內建函式可用來計算檔案的年齡排名。 下節將詳細說明年齡排名函式的一般使用。 最後，給出了一個實例，說明了年齡排序的特點。

## 在「新增排名規則」頁面或「編輯排名規則」頁面上使用search_get_age_rank() {#section_34BC5276F2AB4419AD92872A397CA0AF}

指定 `search_get_age_rank()` 如下：

`search_get_age_rank(Birthdate#Half_Life#Default_Rank)`

* 出生日期——檔案的出生日期或開始日期必須是根據欄位的日期格式設定的日期字串。 此日期格式字串必須是欄位引用，如中 `{field_name}`。
...
檔案欄位中包含的日期 `birthdate` 會以時間戳記傳入。 半衰期為28天。 如果日期無效，預設排名值為0.20。

請參閱新增排名規則 [中的選項表格](../c-about-rules-menu/c-about-ranking-rules.md#task_A132789FD4E5423DAD090DCDA7311E8A)。

在「新增排名規則」頁面或「編輯排名規則」頁面的「值／排名」區段中，您只能與自訂 `search_get_age_rank` 的規則搭配使用。 因此，請確定您從「值 **** /排名」下拉式清單中選擇「自訂」。 當規則使用年齡排名函式時，規則的值部分不允許空格。 請確定函式引數中或它們之間沒有空格。 此外，任何數學或條件運算子之間都沒有空格。
...
在處理每個文檔時，中的 `other_field` 值將使用欄位的日期格式定義轉換為Unix的「劃分」表單。 此值用於呼 `search_get_age_rank()` 叫。 由於每個「紀元」值都小於99999999999（至少目前），規則只提供函式的返回值（從0到1）作為排名。

在上述兩個範例中，規則的「資料來源」通常與函式中使用的欄位 `search_get_age_rank()` 相同- `other_field` 在本例中。

## 將年齡排名整合到排名規則中的一個實例 {#section_A86D909687CC45E19D4EA7A4A9283F28}
...
這條規則做了幾件事。 規則的權重設為10。 排名值只是年齡——排名函式的結果，即0到1的值。 不能將空格與一起使用 `search_get_age_rank()`。 此外，請注意，欄位「出生日期」會以大括弧括住。 最後，儲存此規則時，「值／排名」定義中的逗號會取代為字 `#` 元；這種行為是正常的。

**檢視結果**

使用「資料檢視」功能，快速查看年齡排名函式的結果。 新增適當的中繼資料欄位。 在示例中， `age_val` 和 `myrank` 是應添加到資料視圖的兩個元資料欄位。 欄位 `myrank` 會顯示年齡排名如何影響排名值。 該 `age_val` 欄位顯示該文檔的指數衰減函式的原始輸出。

## 預設值 {#section_CB109EF78F914E92955D512ACFC3310E}

以下是與函式相關的三個預設值 `search_get_age_rank()`:

* 可輸入到函式本身的缺 `search_get_age_rank()` 省值。
* 排名規則的預設排名值。
* 中繼資料定義的預設值。

根據故障發生的位置，您可能會得到不同的預設值。

例如，如果正確實作「排名和篩選」，則中繼資料定義的預設值永遠不會發生。 當沒有該中繼資料欄位的有效或已知內容時，此預設值是最後的度假值。 當引用其自己的關聯標籤且標籤在文 `search_get_age_rank()` 檔中缺失時，排名規則中的預設值可能會出現。 在此情況下，此規則會直接移至規則的預設值。 如果年齡排名函式參考其他排名規則的標籤，則如果參考的標籤遺失或無效，則可能會使用傳遞至該年齡排名函式的預設值。

## 新增排名規則 {#task_A132789FD4E5423DAD090DCDA7311E8A}

您可以新 [!DNL Ranking Rules] 增以根據每個網頁的內容，影響網頁在搜尋結果中的相對位置。
...
1. （可選）如果您已建立規則群組並新增規則至群組，請在下拉式清單的頁 [!DNL Define Ranking Rules] 面上 **[!UICONTROL Select Rule Group]** ，選取包含您要編輯之規則的規則群組。
...
1. 在頁面 [!DNL Add Ranking Rule] 上，設定您想要的選項。 標有星號(*)的欄位為必填欄位。
   ...
   請參 [閱編輯報表套裝的Adobe Analytics量度](../c-about-settings-menu/c-about-adobe-analytics-menu.md#task_360904CCBBB140238ADA036C3CC07664)。
   ...
   <table> 
    ...
    <tbody> 
      ...
      <td colname="col2"> <p>如果您選 <span class="uicontrol"> 擇「中繼標 </span> 記」作為「資料來源類型」，則此為網站頁面內找到的中繼標籤名稱。 下拉式選單中的名稱來自已定義中繼資料值的清單，這些值是在「設定&gt;中繼資料&gt;定義」中設定的。 </p> <p>請參 <a scope="local" href="../c-about-settings-menu/c-about-metadata-menu.md#task_6DF188C0FC7F4831A4444CA9AFA615E5" type="task" format="dita"> 閱新增中繼標籤欄 </a>位。 </p> <p>如果您選擇「Adobe Analytics量度（數字）」作為「資料來源類型」，則此為Adobe Analytics量度的名稱。 下拉式選單中的名稱來自已定義的可用Adobe Analytics量度清單，這些量度是在「設定&gt; Adobe Analytics &gt;量度&gt;編輯」中設定的。 </p> <p>請參 <a href="../c-about-settings-menu/c-about-adobe-analytics-menu.md#task_360904CCBBB140238ADA036C3CC07664" type="task" format="dita" scope="local"> 閱編輯報表套裝的Adobe Analytics量度 </a>。 </p> <p>如果您選取的Adobe Analytics量度名稱尚未在「設定&gt;中繼資料 <span class="uicontrol"> &gt;定義」中定義 </span> ，則會 <span class="uicontrol"></span><span class="uicontrol"></span>顯示文字欄位和「新增」按鈕。 輸入中繼資料欄位名稱（中繼資料欄位名稱不能超過20個字元），然後按一下「新 <span class="uicontrol"> 增」 </span>。 </p> <p>當頁面與多個Adobe Analytics索引鍵（例如顯示多個產品的產品頁面）一起出現時，「組合配置」會指定如何處理與該頁面相關聯的多個Adobe Analytics量度值。 選擇以下選項之一： </p> <p> 
      ...
      <td colname="col2"> <p>包含簡單、單一的規則權重編號，或規則權重編號與測試條件的成對清單。 </p> <p>規則權重數是1-10的值，可指出此排名規則相對於其他排名規則在決定檔案的整體排名時的重要性。 規則權重越高，表示重要性越高。 零(0)的權重會忽略規則。 </p> <p>從下 <span class="uicontrol"> 拉式清 </span> 單中選擇「自訂」，以定義規則權重／測試條件配對清單，自訂各頁面的規則權重。 測試條件是用於測試「資料來源值」的Perl片段，或是在自訂篩選指令碼中定義的全域變數（例如，價格、品牌、季節或頁面檢視，如下例所示）。 如果測試條件評估為「true」，則會套用相關的規則權重值。 如果測試條件評估為"false"，則會評估清單中的下一個條件。 <code> 0&nbsp;({price}&nbsp;&gt;&nbsp;50.00)&nbsp;&amp;&amp;&nbsp;({brand}=~/Kuhl/)5&nbsp;{season}&nbsp;=~&nbsp;/Fall/10&nbsp;{pageviews}&nbsp;&gt;&nbsp;1000005 </code>在上述自訂建立的加權／條件範例中，如果第一個測試條件評估為"true"，則套用規則加權0。 即價格包含大於50的值，而品牌包含「Kuhl」)。 如果第一個測試條件評估為"false"，則會評估下一個條件。 如果未滿足任何先前條件，則會指派規則權重5。 </p> <p>您應一律在清單結尾處提供沒有條件的規則權重。 如果您不這麼做，則規則的權重為0，在條件測試未評估為"true"時。 </p> </td> 
      </tr> 
      <tr> 
      <td colname="col1"> <p>值／排名 </p> </td> 
      <td colname="col2"> <p>由其中一個內建的排名函式或可能的資料來源內容以及所需的排名組成。 </p> <p>如果您選 <span class="uicontrol"> 擇「Adobe Analytics量度（數字）」 </span> 作為「資料來源類型」，則會顯示下拉式清單，其中包含下列選項： 
...
1. 按一下 **[!UICONTROL Add]**.
1. 若要預覽規則新增結果，請按一下以 **[!UICONTROL regenerate your staged site index]** 重建分段網站索引。
...
1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. （可選）如果您已建立規則群組並新增任何規則至群組，請在下拉式清單的頁 **[!UICONTROL Define Ranking Rules]** 面上 **[!UICONTROL Select Rule Group]** ，選取包含您要編輯之規則的規則群組。

   請參 [閱新增排名規則群組](../c-about-rules-menu/c-about-ranking-rules.md#task_B65081B3CC9E4330A7FEE77B7BCD36C8)。
1. 在表格的欄標題下 **[!UICONTROL Actions]** ，按一下要 **[!UICONTROL Edit]** 變更的資料來源名稱。
1. 在頁 [!DNL Edit Ranking Rule] 面上，設定您想要的選項。 標有星號(*)的欄位為必填欄位。

   請參閱「新增排名規則」 [下方的選項表](../c-about-rules-menu/c-about-ranking-rules.md#task_A132789FD4E5423DAD090DCDA7311E8A)。
...
1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. （可選）如果您已建立規則群組並新增任何規則至群組，請在下拉式清單的頁 [!DNL Define Ranking Rules] 面上 **[!UICONTROL Select Rule Group]** ，選取包含您要刪除之規則的規則群組。
1. 在表格的欄標題下 **[!UICONTROL Actions]** ，按一下要 **[!UICONTROL Delete]** 變更的資料來源名稱。
1. 在頁面上 [!DNL Delete Ranking Rule] ，按一下 **[!UICONTROL Delete]**。

   您會返回頁 [!DNL Define Ranking Rules] 面。
...
請參 [閱編輯排名規則群組](../c-about-rules-menu/c-about-ranking-rules.md#task_D3EC0437E47144BC9E754FEF99C725E5)。

請參 [閱刪除排名規則群組](../c-about-rules-menu/c-about-ranking-rules.md#task_BE5BE01F377843BEA9846E094A07F2EA)。

請參 [閱檢閱排名規則群組](../c-about-rules-menu/c-about-ranking-rules.md#task_5694459D050C4254A25186038CD66B6E)。

**若要新增排名規則群組**

1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. 在頁 [!DNL Define Ranking Rules] 面上，按一下下拉式清 **[!UICONTROL Select Rule Group]** 單右側的 **[!UICONTROL Add]**。
1. 在頁 [!DNL Add Ranking Rule Group] 面的欄位中， **[!UICONTROL Rule Group Name]** 為新規則群組輸入唯一名稱。
1. 在下拉 **[!UICONTROL Rank Field Name]** 式清單中，選取您要與新規則群組關聯的排名中繼資料欄位名稱。 如果您 **[!UICONTROL None]** 不想指派排名，請選取。

   排名欄位名稱清單來自在> >中新增的中繼資料 **[!UICONTROL Settings]** 定 **[!UICONTROL Metadata]** 義 **[!UICONTROL Definitions]**。
...
1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. 在頁 [!DNL Define Ranking Rules] 面上，按一下下拉式清 **[!UICONTROL Select Rule Group]** 單右側的 **[!UICONTROL Edit]**。
1. 在頁 [!DNL Edit Ranking Rule Group] 面的欄位中， **[!UICONTROL Rule Group Name]** 輸入規則群組的唯一名稱。
1. 在下拉 **[!UICONTROL Rank Field Name]** 式清單中，選取您要與規則群組關聯的排名中繼資料欄位名稱。 如果您 **[!UICONTROL None]** 不想指派排名，請選取。
...
1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. 在頁 [!DNL Define Ranking Rules] 面的下拉式清 **[!UICONTROL Select Rule Group]** 單中，選取您要刪除的群組。
1. 在下拉式清單 **[!UICONTROL Select Rule Group]** 的右側，按一下 **[!UICONTROL Delete]**。
1. 在頁面上 [!DNL Delete Ranking Rule Group] ，按一下 **[!UICONTROL Delete]**。
1. 重建分段網站索引以預覽新增規則的結果。
...
1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. 在頁 [!DNL Define Ranking Rules] 面上，按一下下拉式清 **[!UICONTROL Select Rule Group]** 單右側的 **[!UICONTROL Overview]**。
1. 在頁面 [!DNL Ranking Rule Groups Overview] 上，按一 **[!UICONTROL Close]** 下以返回頁 [!DNL Define Ranking Rules] 面。
1. （可選）執行下列任一項作業：
...
1. 在產品功能表上，按一下 **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**。
1. 在頁 [!DNL Define Ranking Rules] 面上，在區 **[!UICONTROL Test URL]** 域中，輸入網站上網頁的URL。
...
## 調整與排名規則相關的權重 {#task_3CB6FC92A66F4D99874A42D55825DB64}

您可以變更個別排名規則的相對貢獻，以及排名對最終搜尋結果的貢獻。

未定義「排名」時，搜尋結果會100%位於頁 **[!UICONTROL Natural Relevance]** 面上的「規則與相關性」滑桿 **[!UICONTROL Adjust Ranking Weights]** 旁。 這種平衡只表示搜尋結果僅依搜尋詞排序。
...
1. 在頁 [!DNL Adjust Ranking Weights] 面的下拉式清 **[!UICONTROL Select Rule Group]** 單中，選取您要調整其排名權重的群組。
1. 拖曳滑桿以變更對應的貢獻值。
