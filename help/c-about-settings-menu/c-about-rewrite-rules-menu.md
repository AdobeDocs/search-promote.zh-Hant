...
變數：這些變數的格式為%{NAME_OF_VARIABLE}，其中NAME_OF_VARIABLE是已定義變數名稱的字串。 有關設 `*[E]*` 置環境變數的詳細資訊，請參閱標籤。
...
1. 「替代 `(https://${tolower:$1}$2)` 」會通知搜尋引擎使用第一個回參考上的 `tolower` 函式重寫URL, `(https:// ${tolower:$1}$2)` 而未改變其餘的URL `(https://${tolower:$1} $2)`。
...
變數：這些變數的格式為%{NAME_OF_VARIABLE}，其中NAME_OF_VARIABLE可以是已定義變數名稱的字串。 如需設定變數的 *`[E]`* 詳細資訊，請參閱「重寫規則」標幟。
...
RewriteRule模式包含兩個反向參照： `(.+)` 和 `(.*)`。 第一個回參考會比對之前的所有字元 `sessionid`。 第二個反向引用與終止或之後的所有字元 `&` 匹配 `#`。

「替代」模式使用第一個回參考，接著字串&quot;sessionid=&quot;，接著第一個規則所定義的作業階段ID變數值，接著第二個回參考，來重寫 `%{sessionid}`URL。 `($1sessionid=%{sessionid} $2)`

請注意，此RewriteRule不包含RewriteCond。 因此，會對所有符合「重寫規則模式」的URL進行重 *寫*。 因此，如果sessionid變數( `%{sessionid}`)的值 `1234`為，表單的URL會重寫 `https://www.domain.com/products/?sessionid=5678&function=buy` 為 `https://www.domain.com/products/?sessionid=1234&function=buy`
...
1. 在欄位 [!DNL Crawl List Store URL Rules] 中，輸入您要的規則。

   允許以「#」（雜湊）字元開頭的空白行和註解行。
1. （可選）在頁 [!DNL Crawl List Store URL Rules] 面的欄位中，輸 [!DNL Test Crawl List Store URL Rules] 入您要測試其編目規則的測試URL，然後按一下「測 **試」**。
1. 按一下&#x200B;**「儲存變更」**。
...
1. （可選）在頁 [!DNL Crawl List Store URL Rules] 面上，執行下列任一項作業：
...
變數：這些變數的格式為%{NAME_OF_VARIABLE}，其中NAME_OF_VARIABLE是已定義變數名稱的字串。 有關設定 *[環境變數的詳細資訊]* ，請參閱E標籤。
...
規則運 `^&#]+` 算式會比對單字與下一個***或 `sessionid` #******字元之間的URL部分。 由於此RewriteRule僅用於為sessionid變數建立初始值，因此不會重寫。 請注意，規則的「替代」欄位設定為 `-` 指示不需要重寫。
...
## 新增編目清單擷取URL規則 {#task_94A28ED7DC404BFF9767DBB5ADEE6B7A}
...
1. 在欄位 [!DNL Crawl List Retrieve URL Rules] 中，輸入您要的規則。

   允許以「#」（雜湊）字元開頭的空白行和註解行。
1. （可選）在頁 [!DNL Crawl List Retrieve URL Rules] 面的欄位中，輸 [!DNL Test Crawl List Retrieve URL Rules] 入您要測試其編目規則的測試URL，然後按一下「測 **試」**。
1. 按一下&#x200B;**「儲存變更」**。
1. （可選）如果要預覽結果，請重建分段網站索引。

   請參 [閱配置分段網站的增量索引](../c-about-index-menu/c-about-incremental-index.md#task_46A367B0786C4C90BFFA5D3F95FD86C0)。
1. （可選）在頁 [!DNL Crawl List Retrieve URL Rules] 面上，執行下列任一項作業：
...
變數這些為%{NAME_OF_VARIABLE}格式的變數，其中NAME_OF_VARIABLE可以是定義變數名稱的字串。 有關設 `[E]` 置環境變數的詳細資訊，請參閱標籤。
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> '&lt;CondPattern' </p> </td> 
   <td colname="col2"> <p>從詞法上來說，它就更少了。 </p> <p>將 <i>CondPattern</i> 視為純字串，並從語義上與 <i>TestString比較</i>。 如果 <i>TestString</i> 在詞法上小於 <i>CondPattern</i>，則為true。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> '&gt;CondPattern' </p> </td> 
   <td colname="col2"> <p>詞法上更大。 </p> <p>將 <i>CondPattern</i> 視為純字串，並從語義上與 <i>TestString比較</i>。 如果 <i>TestString</i> 凌駕於 <i>CondPattern上，則</i>為true。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> '=CondPattern' </p> </td> 
   <td colname="col2"> <p> 從詞法上講是相等的。 </p> <p>將 <i>CondPattern</i> 視為純字串，並從語義上與 <i>TestString比較</i>。 如果 <i>TestString</i> 在詞法上等於 <i>CondPattern</i>，即兩個字串完全相同（字元逐字元），則為true。 如 <i>果CondPattern</i> 僅為""（兩個引號），則會比較 <i>TestString</i> 和空字串。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> 'nocase|NC' </p> </td> 
   <td colname="col2"> <p>沒案子。 </p> <p> 使測試不敏感。 也就是說，在擴充的 <i>TestString和</i> CondPattern中，'A-Z'和'a-z'之間沒有差 <i>別。</i> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 'ornext|OR' </p> </td> 
   <td colname="col2"> <p> 或者下一個條件。 </p> <p>使用此標幟，將規則條件與本機OR（而非隱式AND）結合。 若沒有此標幟，您必須多次寫入第二個／規則。 </p> </td> 
...
1. 在欄位 [!DNL Crawl Title Rules] 中，輸入您要的規則。

   允許以「#」（雜湊）字元開頭的空白行和註解行。
1. （可選）在頁 [!DNL Crawl Title Rules] 面的欄位中， [!DNL Test Crawl Title Rules] 輸入您要測試其搜尋規則的測試URL，然後按一下「測 **試」**。
1. 按一下&#x200B;**「儲存變更」**。
1. （可選）如果要預覽結果，請重建分段網站索引。

   請參 [閱配置分段網站的增量索引](../c-about-index-menu/c-about-incremental-index.md#task_46A367B0786C4C90BFFA5D3F95FD86C0)。
1. （可選）在頁 [!DNL Crawl Title Rules] 面上，執行下列任一項作業：
...
1. 「替代 `(https://${tolower:$1}$2)` 」會通知搜尋引擎使用第一個回參考上的 **tolower** 函式來重寫URL, `(https://**${tolower:$1**}$2)` 而URL的其餘部分則保持不變 `(https://${tolower:$1}*$2*)`。
...
變數這些為%{NAME_OF_VARIABLE}格式的變數，其中NAME_OF_VARIABLE可以是定義變數名稱的字串。 如需設定變數的 *`[E]`* 詳細資訊，請參閱「重寫規則」標幟。

>[!NOTE]
>
>重寫規則通常使用變數。 來自目前URL的所有CGI參數都會自動設為變數。 例如，搜尋URL會自 `"https://search.atomz.com/search/?sp_a=sp00000000&sp_q="Product"&session=1234&id=5678"` 動提供4個變數，可在重寫規則中參考。 在此範例中，一個變數稱為「session」，其值為「1234」，而另一個變數稱為「id」，其值為「5678」。 (其他兩個變數 `sp_a` 為 `sp_q`和。)您應從網頁的搜尋表單中，將所有必要的變數傳遞為隱藏欄位。 在此範例中，您應傳遞「session」和「id」值，以識別執行搜尋的網站使用者。 若要在搜尋表單上傳遞隱藏欄位，請使用類似的標籤 `<input type=hidden name="session" value="1234">`。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> '=CondPattern' </p> </td> 
   <td colname="col2"> <p>從詞法上講是相等的。 </p> <p> 將 <i>CondPattern</i> 視為純字串，並從語義上與 <i>TestString比較</i>。 如果 <i>TestString</i> 在詞法上等於 <i>CondPattern</i>，則為true。 也就是說，兩個字串完全相同（字元逐字元）。 如 <i>果CondPattern</i> 僅為""（兩個引號），則會比較 <i>TestString</i> 和空字串。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
&#39;nocase|NC&#39;（無大寫）:這會讓測試不區分大小寫。 換言之，在擴充的 *TestString* 和 *CondPattern中，&#39;A-Z&#39;和&#39;a-z&#39;之間沒有差異*。
...
1. 在欄位 [!DNL Search URL Rules] 中，輸入您要的規則。

   允許以「#」（雜湊）字元開頭的空白行和註解行。
1. （可選）在頁 [!DNL Search URL Rules] 面的欄位中，輸 [!DNL Test Search URL Rules] 入您要測試其編目規則的測試URL，然後按一下「測 **試」**。
1. 按一下&#x200B;**「儲存變更」**。
1. （可選）如果要預覽結果，請重建分段網站索引。

   請參 [閱配置分段網站的增量索引](../c-about-index-menu/c-about-incremental-index.md#task_46A367B0786C4C90BFFA5D3F95FD86C0)。
1. （可選）在頁 [!DNL Search URL Rules] 面上，執行下列任一項作業：
...
## 關於搜尋標題規則 {#concept_C72D20F8DFF64EDE809AF4B72797E858}
...
**變數** ：這些變數為%{NAME_OF_VARIABLE}格式的變數，其中NAME_OF_VARIABLE可以是已定義變數名稱的字串。 有關設定 [環境變數的詳細資訊] ，請參閱E標籤。 變數也可在產生搜尋結果的搜尋表單中定義。
...
**變數** ：這些變數為%{NAME_OF_VARIABLE}格式的變數，其中NAME_OF_VARIABLE可以是已定義變數名稱的字串。 有關設 `[E]` 置環境變數的詳細資訊，請參閱標籤。 變數也可在產生搜尋結果的搜尋表單中定義。

**函式** ：以下是形式為${NAME_OF_FUNCTION:key}的函式，其中NAME_OF_FUNCTION為：
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>使測試不敏感。 也就是說，在擴充的 <i>TestString和</i> CondPattern中，'A-Z'和'a-z <i>'之間沒有差</i>別。 </p> </td> 
  </tr> 
  <tr> 
...
1. 在欄位 [!DNL Search Title Rules] 中，輸入您要的規則。

   允許以「#」（雜湊）字元開頭的空白行和註解行。
1. （可選）在頁 [!DNL Search Title Rules] 面上，在欄 [!DNL Test Search Title Rules] 位中輸入測試標題，然後按一下 **測試**。
1. 按一下&#x200B;**「儲存變更」**。
1. （可選）如果要預覽結果，請重建分段網站索引。

   請參 [閱配置分段網站的增量索引](../c-about-index-menu/c-about-incremental-index.md#task_46A367B0786C4C90BFFA5D3F95FD86C0)。
1. （可選）在頁 [!DNL Search Title Rules] 面上，執行下列任一項作業：
