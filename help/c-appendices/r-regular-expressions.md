...
* 反斜線(\)後面跟任何特殊字元是符合特殊字元本身的單字元規則運算式。 特殊字元包括：
   ...
   * `[ ]`（左方括弧和右方括弧）中包含的非空字串是一個單字元規則運算式，它匹配該字串中的一個字元，包括補充代碼集字元。
      ...
      指定範圍的字元必須來自相同的程式碼集。 當字元來自不同的程式碼集時，會比對指定範圍的其中一個字元。 如果`-`是在字串中出現的第一個（在初始`^`之後，如果有的話）或最後一個，則&lt;a0/>會遺失此特殊含義。 當`]`（右方括弧）是字串中的第一個字元時，若有，`^`初始字元後不會終止此字串。 例如，`[]a-f]`與`]`（右方括弧）或其中一個ASCII字母a到f（含）匹配。 上面列為特殊字元的四個字元，代表在這樣一串字元中。
...
* 一字元規則運算式，後面接著`{m}`、`{m,}`或`{m,n}`，是符合單字元規則運算式發生範圍的規則運算式。 m和n的值必須是小於256的非負整數；`{m}`完全符合m次；`{m,}`至少符合m次；`{m,n}`符合m和n（含）之間的任意次數。 每當有選項存在時，規則運算式會盡可能多地匹配。
* 規則運算式的串連是規則運算式，與規則運算式中每個元件所匹配之字串的串連相符。
* 字元序列（和）之間包含的規則表達式是與任何樸素規則運算式匹配的規則運算式。
