...
| 排序 | `sort=number` | 指定排序順序。<br>「0」是預設值，並依關聯分數排序；「1」按日期排序；&quot;-1&quot;不排序。  使用者可以指定參數值的欄位 `sp_s` 名稱。  例如， `sp_s=title` 根據標題欄位中包含的值對結果排序。 當使用欄位名稱做為參數值時，結果 ` sp_s ` 會依該欄位排序，然後依相關性子排序。  To enable this feature, click [!UICONTROL Settings ] > [!UICONTROL Metadata ] > [!UICONTROL Definitions ]. 在「定義」頁面上，按一 [!UICONTROL Add New Field ] 下或按一 [!UICONTROL Edit ] 下特定欄位名稱。 在下拉 [!UICONTROL Sorting ] 式清單中，選取或 [!UICONTROL Ascending ] 選取 [!UICONTROL Descending ]。 此參數會對應至後端 `sp_s` 搜尋參數。 <br>請參 [閱後端搜尋CGI參數]。(../c-appendices/c-cgiparameters.md#reference_582E85C3886740C98FE88CA9DF7918E8)。 |
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p>如果 <code>sp_advanced=1 </code> 隨查詢提交，則搜尋範本中標 <code>&lt;search-if-advanced&gt; </code> 簽和標 <code>&lt;/search-if-advanced&gt; </code> 簽之間的所有代碼都會用於搜尋表單。 標籤和標籤之 <code>&lt;search-if-not-advanced&gt; </code> 間的所有代碼 <code>&lt;/search-if-not-advanced&gt; </code> 都會被忽略。 如 <code>sp_advanced=0 </code> 果（或任何其他值）已提交，則會忽略&lt;search-if-advanced&gt;範本區塊，並使用&lt;search-if-not-advanced&gt;範本區塊。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p>收集指定欄位的內容相關資訊。 收集的資訊會透過範本標籤輸出在搜尋結果 <code>&lt;search-context&gt; </code> 中。 預設值為 <code>body </code>。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>指定要執行的日期範圍搜尋的類型。 類型的可能值為任何，這表示不執行日期範圍搜尋、自訂，指出應使用的值來判斷要搜尋的日期，而 <code>sp_date_range </code> 且具體指出 <code>sp_start_day </code>、 <code>sp_start_month </code>、 <code>sp_start_year </code>、 <code>sp_end_day </code><code>sp_end_month </code><code>sp_end_year </code> 和中的值用來判斷要搜尋的日期範圍。 <code>sp_d </code> 只有在您的搜尋表單包含依自訂範圍(依方式 <code>sp_date_range </code>)或依特定開始和結束日期範圍進行搜尋的選項時，才需要此選項。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>指定要對相應查詢執行的日期範圍搜索的 <code>sp_q_# </code> 類型。 "#"會以1到16之間的數字取代(例如， <code>sp_d_8 </code>套用至編號的查詢 <code>sp_q_8 </code>)。 </p> <p>您可以設 <code>type </code> 置為任何，這表示不執行日期範圍搜索、自定義，這表示值用於確定要搜索的日期，並且具體表示應使用 <code>sp_date_range_# </code> 、 <code>sp_q_min_day_# </code>、 <code>sp_q_min_month_# </code><code>sp_q_min_year_# </code><code>sp_q_max_day_# </code><code>sp_q_max_month_# </code><code>sp_q_max_year_# </code> 、、和中的值來確定日期範圍。 只有當您 <code>sp_d_# </code> 的搜尋表單包含依自訂範圍(透過方式 <code>sp_date_range_# </code>)或特定開始和結束日期範圍進行搜尋的選項時，才需要使用。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p>指定一個欄位，用於刪除上的重複搜索結果。 該欄位上的所有重複結果都會從搜尋結果中移除。 例如，若為， <code>sp_dedupe_field=title </code>則搜尋結果中只會顯示指定標題的最上方結果（沒有兩個結果會有相同的標題欄位內容）。 對於多值（允許清單）類型欄位，將使用整個欄位內容進行比較。 只能指定一個欄位。 欄位名稱中不允許使用「表限定詞」。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>指定應針對查詢字串中具有數字以上字元的任何字詞，進行自動萬用字元擴增。 換句話說， <code>sp_e=5 </code> 指定應以萬用字元'*'來擴充包含5個或多個字元的字詞，例如"query"或"number"，使搜尋等同於搜尋"query*"或"number*"。 字元數較少的字詞不會展開，因此搜尋「word」時不會自動擴充萬用字元。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>指定自動通配符擴展對於具有多個數字字元的對 <code>sp_q_# </code> 應查詢字串中的任何字。 換言之，指 <code>sp_e_2=5 </code> 定查詢字串中包含5個以上字元的字詞（例如「query」或「number」）應以萬用字元「 <code>sp_q_2 </code><code>* </code>」展開，使搜尋等同於搜尋「query*」或「number*」。 字元數較少的字詞不會展開，因此在中搜尋「word」時， <code>sp_q_2 </code> 不會自動擴充萬用字元。 </p> </td> 
  </tr> 
  <tr> 
